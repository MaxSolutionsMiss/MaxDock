(function(){
  ...
  function getLandingPage(roleCode=state.profile?.role_code){
    if(isVendorProfile())return "my-appointments.html?v=93-db71";
    if(["shipping_manager","coordinator"].includes(roleCode))return "queue.html?v=93-db71";
    if(["system_admin","site_admin"].includes(roleCode))return "dashboard.html?v=93-db71";
    return "index.html?v=93-db71";
  }
  ...
  function addAccountControls(){
    ...
    if(!wrap){
      ...
      const bell=document.createElement("a");bell.id="maxdockNotificationBell";bell.className="notificationBell";bell.href="./my-appointments.html?v=93-db71";bell.title="Open notifications";bell.setAttribute("aria-label","Open notifications");
