(function(){
  ...
  function queueCard(item){
    ...
    const historyLink=!item.linkedMovement&&db.hasPermission("audit.view")?`<a class="secondaryBtn queueCardUtility" href="./dashboard.html?v=64-db43&amp;date=${esc($("queueDate").value)}&amp;history=${esc(item.id)}">History</a>`:"";
  ...
  function renderFocus(rows){
    ...
    $("queueFocus").innerHTML=`<div><small>Next operational focus · ${esc(displayDate($("queueDate").value))}</small><h3><span>${esc(displayTime(next.start))}</span> ${esc(next.direction)} · ${esc(next.ref)}</h3><p>${esc(next.company)} · ${esc(next.truck)} · ${Number(next.skids||0)} skids · ${esc(next.dock)}</p></div><a class="secondaryBtn actionBtn" href="./dashboard.html?v=64-db43&date=${esc($("queueDate").value)}">Open schedule</a>`;
  }
