(function(){
  ...
  function queueCard(item){
    ...
    const tag=item.linkedMovement?"Linked movement":completed?"Completed":level==="overdue"?"Time passed":level==="soon"?"Due within 60 min":item.priority?"Priority":"Planned";
    const canChange=!item.linkedMovement&&(completed?db.hasPermission("appointment.update"):db.hasPermission("appointment.complete"));
    const historyLink=!item.linkedMovement&&db.hasPermission("audit.view")?`<a class="secondaryBtn queueCardUtility" href="./dashboard.html?v=47-db26&amp;date=${esc($("queueDate").value)}&amp;history=${esc(item.id)}">History</a>`:"";
    return `<article class="queueCard ${level} ${item.linkedMovement?"linkedMovement":""}">
      ...
      <div class="queueCardBody">
        <div class="queueCardTop"><b>${esc(item.ref)}</b><span class="queueTag">${esc(item.linkedMovement?"Linked movement":item.afterHours?"After hours":tag)}</span></div>
        ...
        <div class="queueCardFooter"><span class="queueState ${completed?"completed":"pending"}">${item.linkedMovement?"Read-only site view":completed?"Completed":`Pending • ${esc(item.status)}`}</span><div class="queueCardActions">${historyLink}${canChange?`<button class="${completed?"secondaryBtn":"greenBtn"} queueStatusAction" type="button" data-queue-id="${esc(item.id)}" data-queue-status="${completed?"scheduled":"completed"}" ${state.busyId===item.id?"disabled":""}>${state.busyId===item.id?"Saving…":completed?"Reopen as Pending":"Mark Complete"}</button>`:""}</div></div>
      </div>
    </article>`;
  }
  ...
  function renderFocus(rows){
    ...
    $("queueFocus").innerHTML=`<div><small>Next operational focus · ${esc(displayDate($("queueDate").value))}</small><h3><span>${esc(displayTime(next.start))}</span> ${esc(next.direction)} · ${esc(next.ref)}</h3><p>${esc(next.company)} · ${esc(next.truck)} · ${Number(next.skids||0)} skids · ${esc(next.dock)}</p></div><a class="secondaryBtn actionBtn" href="./dashboard.html?v=47-db26&date=${esc($("queueDate").value)}">Open schedule</a>`;
  }
